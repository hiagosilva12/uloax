import styled from "styled-components";

export const ContainerFooterConteudo = styled.div``;

export const ContainerFooter = styled.div`
  width: 100%;
  background-color: #292828;
  height: 61vh;
  padding-top: 30px;
  padding-bottom: 30px;
  margin-top: 90px;
`;

export const SecaoFooter = styled.div`
  width: 100%;
  padding-right: 15px;
  padding-left: 15px;
  margin-right: auto;
  margin-left: auto;
  padding-top: 40px;
`;

export const BoxFooter = styled.div`
  display: flex;
  flex-wrap: wrap;
  margin-right: 6%;
  margin-left: 6%;
  margin-bottom: 15px;
`;

export const BoxAddress = styled.div`
  flex: 3 0 25%;
  max-width: 23%;
  position: relative;
  width: 100%;
  min-height: 1px;
  padding-right: 15px;
  padding-left: 15px;
`;

export const TitleAddress = styled.h1`
  width: 100%;
  float: left;
  font-size: 28px;
  color: #ebebeb;
  font-weight: bold;
  padding: 25px 0px;
`;

export const BoxLinks = styled.div`
  flex: 3 0 25%;
  max-width: 25%;
  position: relative;
  width: 100%;
  min-height: 1px;
  padding-right: 5px;
  padding-left: 45px;
`;

export const TitleFollow = styled.h1`
  width: 100%;
  float: left;
  font-size: 28px;
  color: #ebebeb;
  font-weight: bold;
  padding: 25px 0px;
`;

export const BtnSubscribe = styled.button`
  width: 60%;
  float: left;
  font-size: 16pt;
  color: #eceaea;
  background-color: #fecf3d;
  padding: 11px 0px;
  border: 0px;
  text-align: center;
  margin-top: 20px;
  font-weight: 600;
  border-radius: 5px;
  cursor: pointer;
  &:hover {
    background-color: red;
  }
`;

export const BoxHome = styled.div`
  display: flex;
  margin-top: 4px;
  margin-bottom: 7px;
  width: 100%;
  float: left;
`;

export const ImgBolinha = styled.img`
  margin-top: 5px;
  width: 10px;
  height: 10px;
`;

export const Link = styled.a`
  text-decoration: none;
  color: white;
`;

export const ParagraphCopyright = styled.p`
  width: 100%;
  float: left;
  font-size: 17px;
  color: #000;
  text-align: center;
  height: 3rem;
`;

export const BoxCopyright = styled.div`
  display: flex;
  justify-content: center;
  align-items: flex-start;
  margin-top: 2%;
  width: 100%;
  background-color: #ffffff;
`;

export const BoxRotas = styled.div`
  display: flex;
  margin-top: 4px;
  margin-bottom: 7px;
  width: 100%;
  float: left;
`;

export const ParagraphSociais = styled.p`
  color: #fffcf4;
  margin-left: 17px;
  &:hover {
    color: #fecf3d;
  }
`;

export const BoxConteudoFollow = styled.div`
  display: flex;
  margin-bottom: 7px;
  width: 100%;
  float: left;
`;

export const ImgSociais = styled.img`
  max-width: 100%;
  height: auto;
  border-style: none;
`;

export const Input = styled.input`
  width: 100%;
  float: left;
  font-size: 12pt;
  color: #eceaea;
  background-color: #ffffff;
  padding: 10px;
  border-radius: 5px;
  border: 0px;
`;

export const ParagraphLorem = styled.p`
  font-size: 12pt;
  color: #ebebeb;
  margin-bottom: 20px;
  font-weight: 300;
  line-height: 24px;
`;

export const BoxFollow = styled.div`
  flex: 3 0 25%;
  max-width: 25%;
  position: relative;
  width: 100%;
  min-height: 1px;
  padding-right: 15px;
  padding-left: 15px;
`;

export const BoxNew = styled.div`
  flex: 3 0 25%;
  max-width: 25%;
  position: relative;
  width: 100%;
  min-height: 1px;
  padding-right: 15px;
  padding-left: 15px;
`;
